...
trait Call extends AbstractNode {
  ...
  def checkSlots(params: List[ParamDecl], reporter: SemanticCheckReporter): Unit = {
    if (params.length > args.length) {
      reporter.report(
        Severity.Error, this, s"not enough arguments specified to call function '${ name }': expecting ${
          params
            .length
        }, got ${ args.length }"
        )
    ...
    if (params.length < args.length) {
      reporter.report(
        Severity.Error, this, s"too many arguments specified to call function '${ name }': expecting ${
          params
            .length
        }, got ${ args.length }"
        )
